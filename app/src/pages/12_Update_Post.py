...
if st.button("Update Post"):
        if post_id and title and text:
            # Check if the title or text has actually changed
            if title != preload_post.get('title') or text != preload_post.get('text'): 
                ...
                st.info("No changes detected. Post remains the same.")
        else:
