...
country = st.text_input("Protest Country", value = preload_post['country'])
...
with col1:
    if st.button("Update Protest", use_container_width = True):
        if protest_id and location and description and country and cause:
            if location!= preload_post['location'] or description != preload_post['description'] or country != preload_post['country'] or cause != preload_post['cause_name']:
